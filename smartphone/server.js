...
if (pB.base === 'creative_v3') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (c, d) => {
    ...
    if (pB.base === 'creative_v4') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (d, e) => {
        ...
        } else pB.base === 'creative_rework' && (pB.serverSideProp = true, pp.getUserSource = pp.Source, pp.getUserId = pp.Passport, pp.getInventoryItemAmount = pp.InventoryItemAmount, pp.hasPermission = pp.HasPermission, pp.request = pp.Request, pp.getBankMoney = async c => {
        ...
        if (r.length === 0) {
            ...
            if (p.dev) {
                ...
                for (let B of r) {
                    ...
                    const C = await pM(B);
                    if (!C) continue;
                    emit('smartphone:service_request2', C, o,k,n,p);
                    const D = {};
